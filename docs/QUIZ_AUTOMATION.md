...
- 과목/주제/난이도는 한국 시간 기준 날짜와 시간으로 학년별 로테이션됩니다
- 매시간 생성 묶음에는 킹왕짱(`king`) 난이도 퀴즈가 정확히 1개, 영어 카테고리 퀴즈가 1개 이상 포함됩니다
...
자동 실행(`daily`)은 아래 학년별 주제 풀에서 날짜와 시간 기준으로 주제와 난이도를 고른 뒤 Gemini API로 새 퀴즈를 생성합니다:
